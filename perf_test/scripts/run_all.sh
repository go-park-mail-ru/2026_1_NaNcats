#!/usr/bin/env bash
# Полный воспроизводимый прогон «в три действия» (запуск -> результаты в results/).
# Поднимает стенд, заливает 100k, замеряет чтение baseline, применяет оптимизации
# пошагово и перезамеряет, снимает EXPLAIN'ы.
set -euo pipefail
