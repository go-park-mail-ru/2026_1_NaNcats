...
CREATE INDEX idx_support_events_ticket_id ON support_events(ticket_id);
CREATE INDEX idx_support_tickets_client_id ON support_tickets(client_id) WHERE client_id IS NOT NULL;
CREATE INDEX idx_support_tickets_guest_id ON support_tickets(guest_id) WHERE guest_id IS NOT NULL;
CREATE INDEX idx_support_tickets_status ON support_tickets(current_status);
CREATE INDEX idx_support_tickets_line ON support_tickets(support_line);
CREATE INDEX idx_support_tickets_assignee ON support_tickets(assignee_id) WHERE assignee_id IS NOT NULL;
CREATE INDEX idx_support_agents_status ON support_agent_profiles(status) WHERE status = 'ONLINE';
