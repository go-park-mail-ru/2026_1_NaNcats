...
func TestCartRepo_DowngradeToSolo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock pgxmock.PgxPoolIface) {
				mock.ExpectBegin()
				mock.ExpectExec(`DELETE FROM "cart_member"`).WithArgs(cartID, adminID).WillReturnResult(pgxmock.NewResult("DELETE", 1))
				mock.ExpectExec(`DELETE FROM "cart_dish"`).WithArgs(cartID, adminID).WillReturnResult(pgxmock.NewResult("DELETE", 2))
				mock.ExpectExec(`UPDATE "cart" SET mode`).WithArgs(domain.CartModeSolo, cartID).WillReturnResult(pgxmock.NewResult("UPDATE", 1))
		...
		},
		{
			...
			mock: func(mock pgxmock.PgxPoolIface) {
				mock.ExpectBegin()
				mock.ExpectExec(`DELETE FROM "cart_member"`).WithArgs(cartID, adminID).WillReturnError(errors.New("batch error"))
				mock.ExpectRollback()
			},
